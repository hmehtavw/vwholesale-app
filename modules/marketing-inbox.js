...
function inboxPaintList(threads) {
  ...
  el.innerHTML = threads.map(function (t, i) {
    ...
    return '<div onclick="inboxOpen(' + i + ')" style="display:flex;gap:10px;padding:11px 12px;cursor:pointer;'
      ...
      + (unread ? '<div style="background:#ef4444;color:#fff;border-radius:9px;font-size:9px;padding:1px 6px;margin-top:4px;display:inline-block;font-weight:700">' + unread + '</div>' : (!t.last_inbound_at ? '<div style="font-size:8px;color:var(--text3);margin-top:1px">sent ↗</div>' : ''))
      + '</div></div>';
  }).join('');
}
