...
async function renderCalendar(offsetUnits) {
  ...
  const allFiltered = (calItems||[]).filter(i =>
    (calFilterType==='all' || i.content_type===calFilterType || (calFilterType==='reel' && i.is_reel)) &&
    (calFilterStatus==='all' || i.status===calFilterStatus) &&
    (!i.cal_date || (i.cal_date >= viewStart && i.cal_date <= viewEnd))
  );
  const reelDays = allFiltered.filter(i => i.is_reel || i.content_type==='reel');
  const otherDays = allFiltered.filter(i => !i.is_reel && i.content_type!=='reel');
  ...
  const _sp = (k,l) => '<button onclick="window._calFilterStatus=\''+k+'\';renderCalendar()" style="padding:4px 10px;font-size:11px;font-weight:700;border-radius:20px;border:none;cursor:pointer;background:'+(calFilterStatus===k?'var(--accent)':'transparent')+';color:'+(calFilterStatus===k?'#000':'var(--text2)')+'">'+l+'</button>';
  ...
  <div style="margin-bottom:16px">
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px;flex-wrap:wrap;gap:8px">
      ...
      <div style="display:flex;gap:6px;align-items:center">
        <button onclick="renderCalendar(${offsetUnits-1})" class="mkt-btn mkt-btn-ghost" style="padding:5px 12px;font-size:14px">&#8592;</button>
        <button onclick="window._calOffset=0;renderCalendar(0)" class="mkt-btn mkt-btn-ghost" style="padding:5px 10px;font-size:11px">Today</button>
        <button onclick="renderCalendar(${offsetUnits+1})" class="mkt-btn mkt-btn-ghost" style="padding:5px 12px;font-size:14px">&#8594;</button>
        <button onclick="addCalendarItem()" class="mkt-btn mkt-btn-primary" style="padding:5px 12px;font-size:11px">+ Add</button>
      </div>
    </div>
    ...
    <div style="display:flex;gap:6px;flex-wrap:wrap">
      <div style="display:flex;gap:3px;background:var(--bg2);border-radius:24px;padding:3px">
        ${_tp('all','All')}${_tp('image','\ud83d\uddbc\ufe0f Poster')}${_tp('gif','\ud83c\udfa5 GIF')}${_tp('reel','\ud83d\udcf1 Reel')}${_tp('festival','\ud83c\udf89 Festival')}
      </div>
      <div style="display:flex;gap:3px;background:var(--bg2);border-radius:24px;padding:3px">
        ${_sp('all','All')}${_sp('planned','\u25a1 Draft')}${_sp('ready','\u23f3 Ready')}${_sp('approved','\u2705 Approved')}${_sp('published','\ud83d\udce4 Posted')}
      </div>
    </div>
  </div>
