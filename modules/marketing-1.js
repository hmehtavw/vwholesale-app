        ...
        <div style="border-top:1px solid var(--border);margin-top:8px;padding-top:10px;display:grid;gap:8px">
          ...
          <div style="display:flex;gap:6px;align-items:center;flex-wrap:wrap">
            ...
            ${item.content_type!=='reel'&&item.content_type!=='gif'
              ? `<button onclick="calGeneratePosters('${item.id}',false,'standard')" class="mkt-btn mkt-btn-ghost" style="font-size:11px;padding:6px 12px" title="2 AI backgrounds — story + landscape (recommended)">🤖 Auto Poster</button>
                 <button onclick="calGeneratePosters('${item.id}',false,'premium')" class="mkt-btn mkt-btn-ghost" style="font-size:11px;padding:6px 10px" title="3 dedicated AI backgrounds — festivals, launches, ads">🏆</button>
...
async function calGeneratePosters(calendarId, reuseBg, qualityMode) {
  const mode = qualityMode || 'standard';
  const modeLabel = mode === 'premium' ? '🏆 Premium — 3 dedicated AI backgrounds' : mode === 'economy' ? '⚡ Economy — 1 background, fast' : '⚙️ Standard — 2 format-specific backgrounds';
  const btn = document.querySelector(`button[onclick*="calGeneratePosters('${calendarId}')"]`);
  if (btn) { btn.disabled = true; btn.textContent = '⏳ Generating…'; }
  showMktToast(`⏳ ${modeLabel} (~60-90s)`);
  try {
