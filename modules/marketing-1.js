...
async function renderGBP() {
  ...
  const viewBtns = ['week','fortnight','month','quarter'];
  const typeBtns = [
    {k:'all', label:'All Types'},
    {k:'image', label:'\ud83d\uddbc\ufe0f Poster'},
    {k:'gif', label:'\ud83c\udfa5 GIF'},
    {k:'reel', label:'\ud83d\udcf1 Reel'},
  ...
  const statusBtns = [
    {k:'all', label:'All'},
    {k:'planned', label:'\u25a1 Draft'},
    {k:'ready', label:'\u23f3 Ready'},
    {k:'approved', label:'\u2705 Approved'},
    {k:'published', label:'\ud83d\udce4 Posted'},
  ];
  const pill = (active) => active
    ? 'padding:5px 12px;font-size:11px;font-weight:700;border-radius:20px;border:none;cursor:pointer;background:var(--accent);color:#000'
    : 'padding:5px 12px;font-size:11px;font-weight:600;border-radius:20px;border:none;cursor:pointer;background:transparent;color:var(--text2)';

  setContent(`
  <div style="margin-bottom:16px">
    <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:12px;flex-wrap:wrap;gap:8px">
      <div>
        <h3 style="font-size:18px;font-weight:900;margin-bottom:2px">\ud83d\udcc5 Content Calendar</h3>
        <div style="font-size:12px;color:var(--text3)">${periodLabel} &nbsp;&middot;&nbsp; <strong style="color:var(--text1)">${shownCount}</strong> of ${(calItems||[]).length} posts</div>
      </div>
      <div style="display:flex;gap:6px;align-items:center">
        <button onclick="renderCalendar(${offsetUnits-1})" class="mkt-btn mkt-btn-ghost" style="font-size:14px;padding:6px 14px;border-radius:8px">&larr;</button>
        <button onclick="window._calOffset=0;renderCalendar(0)" class="mkt-btn mkt-btn-ghost" style="font-size:11px;padding:6px 12px;border-radius:8px">Today</button>
        <button onclick="renderCalendar(${offsetUnits+1})" class="mkt-btn mkt-btn-ghost" style="font-size:14px;padding:6px 14px;border-radius:8px">&rarr;</button>
        <button onclick="addCalendarItem()" class="mkt-btn mkt-btn-primary" style="font-size:11px;padding:6px 14px;border-radius:8px">+ Add</button>
      </div>
    </div>
    <div style="display:flex;gap:4px;margin-bottom:10px;flex-wrap:wrap;align-items:center">
      <span style="font-size:10px;font-weight:700;color:var(--text3);margin-right:4px">VIEW</span>
      ${viewBtns.map(v => '<button onclick="window._calViewMode=\''+v+'\';window._calOffset=0;renderCalendar(0)" style="'+pill(calViewMode===v)+'">'+(v==='week'?'This Week':v==='fortnight'?'2 Weeks':v==='month'?'Month':'Quarter')+'</button>').join('')}
    </div>
    <div style="display:flex;gap:8px;flex-wrap:wrap">
      <div style="display:flex;gap:3px;background:var(--bg2);border-radius:24px;padding:4px">
        ${typeBtns.map(t => '<button onclick="window._calFilterType=\''+t.k+'\';renderCalendar()" style="'+pill(calFilterType===t.k)+'">'+t.label+'</button>').join('')}
      </div>
      <div style="display:flex;gap:3px;background:var(--bg2);border-radius:24px;padding:4px">
        ${statusBtns.map(s => '<button onclick="window._calFilterStatus=\''+s.k+'\';renderCalendar()" style="'+pill(calFilterStatus===s.k)+'">'+s.label+'</button>').join('')}
      </div>
    </div>
  </div>
